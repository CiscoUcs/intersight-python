...
try:
    ...
    print(Bcolors.FAIL + "[ERROR]: Import for Intersight SDK modules failed. " + Bcolors.ENDC)
    print(Bcolors.BOLD + "[ERROR-DETAIL]: " + import_error.__str__() + Bcolors.ENDC)
    print(Bcolors.BOLD + "[ERROR-DETAIL]: Try re-installing it: "
                         "'pip install git+https://github.com/CiscoUcs/intersight-python.git'" + Bcolors.ENDC)
    exit()
            ...
            try:
                ...
                if server_moid and HostManager.host_tools_validated(host, os_inv_reader.os_type):
                    ...
                    if env_config.logging:
                        env_config.write_log("[" + host.strip() + "]: Intersight MO Identifier: " + str(server_moid))
                        env_config.write_log("[" + host.strip() + "]: Building OS Inventory Collection... ")
                    ...
                    if intersight_connection.server_tags_differ(os_inv_collection):
                        print(Bcolors.OKBLUE + "[" + host.strip() +
                              "]: Changes detected in OS Inventory, pushing to intersight..." + Bcolors.ENDC)
                        intersight_connection.patch_server_by_moid(host, server_moid, os_inv_collection)
                    else:
                        print(Bcolors.WARNING + "[" + host.strip() +
                              "]: No Changes detected in OS Inventory, skipping..." + Bcolors.ENDC)
                elif not server_moid:
                    print(Bcolors.FAIL + "[" + host.strip() + "]: Server '" + host.strip() +
                          "' look up by SERIAL number did not return any results." + Bcolors.ENDC)
                    print(Bcolors.BOLD + "[ERROR-DETAIL]: Please verify that '" + host.strip() +
                          "' is connected and claimed in Intersight." + Bcolors.ENDC)
                else:
                ...
                print(Bcolors.FAIL + "[ERROR]: Communication with Intersight failed with return code." + Bcolors.ENDC)
                print(Bcolors.BOLD + "[ERROR-DETAIL]: Communication with Intersight failed with response: " +
                      api_error.__str__() + Bcolors.ENDC)
                print(Bcolors.FAIL +
                      "[ERROR]: Could not complete ODT update (Intersight API returned errors) for server: " +
                      host.strip() + Bcolors.ENDC)
                continue

            except subprocess.CalledProcessError as subprocess_error:
                print(Bcolors.BOLD + "[ERROR-DETAIL]: SSH Communication with server failed with response: " +
                      subprocess_error.__str__() + Bcolors.ENDC)
                print(Bcolors.FAIL + "[ERROR]: Could not complete ODT update (ssh communication failed) for server: " +
                      host.strip() + Bcolors.ENDC)
                continue

            except KeyboardInterrupt:
                print(Bcolors.BOLD + "[ERROR-DETAIL]: User initiated ABORT! " + Bcolors.ENDC)
                print(Bcolors.FAIL + "[ERROR]: Something didn't add up! Give the ODT another try?" + Bcolors.ENDC)
                exit()
...
class InvReader:
    ...
    def invoke_shell(self, exec_type, query_type, name):
        ...
        if query_type == QueryType.OS:
            output = subprocess.check_output(['bash', '-c', cmd]).strip()
...
class OsInvReader(InvReader):
    ...
    def get_os_inv(self):
        ...
        os_arch = self.invoke_shell(ExecType.COMMAND, QueryType.OS, " uname -m | awk '{print $1}'")
        os_vendor = (self.invoke_shell(ExecType.SCRIPT, QueryType.OS, "osvendor.sh")).lower().title()
        os_flavor = os_vendor
...
class IntersightConnectionManager:
    ...
    def lookup_server_by_serial(self, hostname):
        ...
        cmd = "ssh -t 2>/dev/null " + hostname.strip() + " sudo /usr/sbin/dmidecode -s system-serial-number"
        host_serial = (subprocess.check_output(['bash', '-c', cmd])).split("\n")
        ...
        print("[" + hostname.strip() + "]: Host Serial Number: " + self.serial_number)
        cmd = "ssh -t 2>/dev/null " + hostname.strip() + " sudo /usr/sbin/dmidecode -s system-product-name"
        host_type = (subprocess.check_output(['bash', '-c', cmd])).split("\n")
        if len(host_type) > 1:
        ...
        if host_serial != "<BAD INDEX>":
            print("[" + hostname.strip() + "]: Extracting Server MO Identity from Intersight... ")
            if 'UCSB' in self.host_type:
                ...
                self.server_api = computeRackUnit.ComputeRackUnitApi(self.api_instance)
                self.results = self.server_api.compute_rack_units_get(inlinecount='allpages',
                                                                      top=100, filter='Serial eq ' + self.serial_number)

            if self.results:
                print("[" + hostname.strip() + "]: Server MO Identity: " + self.results.results[0].moid)
            ...
            print("[" + hostname.strip() + "]: Server SERIAL number invalid: " + self.serial_number)
            return 0
    ...
    def patch_server_by_moid(self, hostname, moid, os_inv_collection):
        ...
        if 'UCSB' in self.host_type:
            ...
            result = self.server_api.compute_rack_units_moid_patch(moid, compute)
        else:
            result = None

        if result is None:
            print("--------------------------------------------------------------")
            print(Bcolors.OKGREEN + "[" + hostname.strip() +
                  "]: OS Inventory push to Intersight completed!" + Bcolors.ENDC)
            print("--------------------------------------------------------------")
        else:
            print("--------------------------------------------------------------")
            print(Bcolors.FAIL + "[" + hostname.strip() +
                  "]: {" + result + "}, OS Inventory push to Intersight failed!" + Bcolors.ENDC)
            print("--------------------------------------------------------------")
        ...
        if len(self.server_tags) != len(os_inv_collection):
            ...
            for new_elem in os_inv_collection:
                if new_elem['Key'] != 'intersight.server.os.updateTimestamp':
                    old_elem = [elem for elem in self.server_tags if elem.to_dict()['key'] == new_elem['Key']][0]
                    if old_elem is None or old_elem.to_dict()['value'] != new_elem['Value']:
                        changed = True
                        break
        return changed
...
class EnvironmentConfigurator:
    ...
    def process_command_args():
        ...
        parser = argparse.ArgumentParser(description=
                                         'ODT: Utility to import OS Inventory of your Linux Environment to Intersight.')
        parser.add_argument("-l", "--log-inventory", action="store_true", dest="logging",
        ...
        if args.configfile is None:
            print(args)
            print(Bcolors.FAIL + "[ERROR]: Please provide MANDATORY arguments" + Bcolors.ENDC)
            print(Bcolors.WARNING + "[HINT]: try $ ./get_linux_inv_to_intersight.py --help" + Bcolors.ENDC)
            exit()
...
def do_discovery():
    ...
    print(Bcolors.OKGREEN + "[ODT: START-TIMESTAMP: " + str(datetime.datetime.utcnow()) + "]" + Bcolors.ENDC)
    ...
    if env_config.logging:
        ...
        env_config.write_log("------------------------START-TIMESTAMP--------------------------")
    else:
        print(Bcolors.WARNING + '[INFO]: Logging disabled...'+ Bcolors.ENDC)
    ...
    print("[INFO]: Found " + str(host_manager.num_hosts) + " hosts in " + env_config.hosts_file)

    intersight_connection = IntersightConnectionManager(env_config.api_key.strip(),
    ...
    print("--------------------------------------------------------------")
    print(Bcolors.BOLD + "[INFO]: ODT push to Intersight completed!" + Bcolors.ENDC)
    print("--------------------------------------------------------------")

    if env_config.logging:
        env_config.write_log("-----------------------END-TIMESTAMP--------------------------")
        env_config.close_log_file()

    print(Bcolors.OKGREEN + "[ODT: END-TIMESTAMP: " + str(datetime.datetime.utcnow()) + "]" + Bcolors.ENDC)
