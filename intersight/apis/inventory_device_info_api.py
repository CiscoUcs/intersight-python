...
class InventoryDeviceInfoApi(object):
    ...
    def inventory_device_infos_get(self, **kwargs):
        ...
        :param int top2: The max number of documents to return.
        :param int skip2: The number of documents to skip.
        :param str filter2: Filter criteria for documents to return. A URI with a $filter System Query Option identifies a subset of the Entries from the Collection of Entries identified by the Resource Path section of the URI. The subset is determined by selecting only the Entries that satisfy the predicate expression specified by the query option. The expression language that is used in $filter operators supports references to properties and literals. The literal values can be strings enclosed in single quotes, numbers and boolean values (true or false) or any of the additional literal representations shown in the Abstract Type System section. Query examples: $filter=Name eq 'Bob' $filter=Tags/any(t: t/Key eq 'Site') $filter=Tags/any(t: t/Key eq 'Site' and t/Value eq 'London') 
        :param str select2: Specifies a subset of properties to return.
        :param str orderby2: Determines what values are used to order a collection of documents.
        :param str expand2: Specify additional attributes or related documents to return. Supports only 'DisplayNames' attribute now. Query examples: $expand=DisplayNames 
        :param str apply2: Specify one or more transformation operations to perform aggregation on documents. The transformations are processed in order with the output from a transformation being used as input for the subsequent transformation. Query examples: $apply=groupby((Model), aggregate($count as Total)) $apply=groupby((Model), aggregate(AvailableMemory with average as AverageAvailableMemory)) 
        :param str at2: Similar to \"$filter\", but \"at\" is specifically used to filter versioning information properties for documents to return. A URI with an \"at\" Query Option identifies a subset of the Entries from the Collection of Entries identified by the Resource Path section of the URI. The subset is determined by selecting only the Entries that satisfy the predicate expression specified by the query option. The expression language that is used in at operators supports references to properties and literals. The literal values can be strings enclosed in single quotes, numbers and boolean values (true or false) or any of the additional literal representations shown in the Abstract Type System section. Query examples: at=VersionType eq 'Configured' at=InterestedMos.Moid eq '5b5877e56c6730367acf46cd' 
    ...
    def inventory_device_infos_get_with_http_info(self, **kwargs):
        ...
        all_params = ['count', 'inlinecount', 'top', 'skip', 'filter', 'select', 'orderby', 'expand', 'apply', 'at', 'count2', 'inlinecount2', 'top2', 'skip2', 'filter2', 'select2', 'orderby2', 'expand2', 'apply2', 'at2']
    ...
    def inventory_device_infos_moid_get(self, moid, moid2, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.inventory_device_infos_moid_get(moid, moid2, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.inventory_device_infos_moid_get_with_http_info(moid, moid2, **kwargs)
        else:
            (data) = self.inventory_device_infos_moid_get_with_http_info(moid, moid2, **kwargs)
            return data

    def inventory_device_infos_moid_get_with_http_info(self, moid, moid2, **kwargs):
        """
        Get a specific instance of 'inventoryDeviceInfo'
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please define a `callback` function
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.inventory_device_infos_moid_get_with_http_info(moid, moid2, callback=callback_function)
